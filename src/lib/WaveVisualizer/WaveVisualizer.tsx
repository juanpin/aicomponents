...
export const WaveVisualizer = forwardRef<WaveVisualizerRef, WaveVisualizerProps>(
    (
        {barColor = '#4a90e2', barGap = 2, height = 100, width = 400, analyzer: initialAnalyzer},
        ref: ForwardedRef<WaveVisualizerRef>
    ) => {
        const canvasRef = useRef<HTMLCanvasElement>(null);
        const animationFrameRef = useRef<number | null>(null);
        const [dataArray, setDataArray] = useState<Uint8Array | null>(null);
        const [analyzer, setAnalyzer] = useState<AnalyserNode | null>(initialAnalyzer || null);
        ...
        return <canvas ref={canvasRef} className={classes.waveVisualizer} width={width} height={height} />;
    }
);
